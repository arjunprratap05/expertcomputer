...
const SidebarBtn = React.memo(({ active, onClick, icon, label }) => (
    <button onClick={onClick} className={`w-full flex items-center gap-4 p-5 rounded-[1.5rem] font-black transition-all group relative ${active ? 'bg-[#F37021] text-white shadow-2xl -translate-x-2' : 'hover:bg-white/5 text-slate-300 hover:text-white hover:translate-x-1'}`}> 
...
export default function AdminDashboard() {
    ...
    // --- NAVIGATION HANDLER (Closes sidebar on mobile) ---
    ...
    const analyzeFinances = useCallback((studentList) => {
        ...
        setFinances({ total: totalRev, monthlyPayout: monthly, pendingAdjustments: pending });
    }, [getNormalizedEnrollments]);
    ...
    ), [activeTab, userRole, handleTabChange]);
        ...
        <div className="flex h-screen bg-slate-50 font-sans overflow-hidden text-left relative text-slate-800">
            ...
            <aside className={`fixed lg:relative z-[200] h-full w-80 bg-[#1A5F7A] text-white p-8 flex flex-col shadow-2xl transition-all duration-500 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="flex justify-between items-start mb-12">
                    ...
                    <button className="lg:hidden text-white/50 p-2 hover:text-white" onClick={() => setIsSidebarOpen(false)}>
                        <FiX size={24} />
                    </button>
                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                <header className="bg-white h-24 px-10 flex items-center justify-between border-b border-slate-100 shadow-sm sticky top-0 z-[100]">
                    <div className="flex items-center gap-5">
                        <button className="lg:hidden text-[#1A5F7A] p-2" onClick={() => setIsSidebarOpen(true)}>
                            <FiMenu size={24} />
                        </button>
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <button onClick={() => setLogoutModal(true)} className="p-4 bg-red-50 text-red-500 rounded-2xl active:scale-90 transition-all">
                            <FiLogOut size={22} />
                        </button>
                    </div>
                </header>
                ...
                <main className="p-6 lg:p-12 overflow-y-auto flex-1 no-scrollbar bg-[#F8FAFC]">
                    {activeTab === 'overview' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
                            ...
                            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 flex items-center gap-5">
                                <div className="p-4 bg-orange-50 text-[#F37021] rounded-2xl"><FiActivity size={30}/></div>
                                <div><p className="text-[10px] font-black text-slate-400 uppercase leading-none mb-1">Monthly</p><div className="text-xl font-black text-[#1A5F7A] italic">₹{finances.monthlyPayout.toLocaleString()}</div></div>
                            </div>
                            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 flex items-center gap-5">
                            ...
                            <div className="bg-white p-8 rounded-[2.5rem] border border-red-100 flex items-center gap-5 border-dashed">
                                                                    ...
                                                                    <div key={i} className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-6 hover:border-[#F37021] transition-all">
                                                                        <div className="flex items-center gap-6">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <div className="flex items-center gap-2 text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-widest"><FiCopy className="cursor-pointer hover:text-[#F37021]" onClick={() => {navigator.clipboard.writeText(en.transactionId); triggerToast("COPIED")}}/>UTR: <span className="text-slate-600 font-black">{en.transactionId}</span></div>
                                                                            </div>
                                                                        </div>
                                                                        <div className="flex items-center gap-8">
                    ...
                    {activeTab === 'coupons' && (
                        <div className="space-y-10">
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden border-t-[10px] border-[#1A5F7A]">
                                ...
                                <form onSubmit={handleCreateCoupon} className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                                    ...
                                    <div className="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-10 mt-6 border-t pt-10">
                                        ...
                                        <div className="col-span-2 space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-4 italic">Value</label><div className="relative"><input required type="number" className="w-full p-8 bg-slate-50 border-2 border-slate-100 rounded-[3rem] font-black text-5xl outline-none text-center focus:bg-white transition-all" value={couponForm.discountValue} onChange={e => setCouponForm({...couponForm, discountValue: e.target.value})} /><div className="absolute right-10 top-1/2 -translate-y-1/2 text-4xl font-black text-slate-200">{couponForm.discountType === 'PERCENTAGE' ? '%' : '₹'}</div></div></div>
                                    </div>
                             ...
                             <div className="bg-[#1A5F7A] rounded-[2.5rem] shadow-xl overflow-hidden">
                                ...
                                <div className="bg-white mx-1 mb-1 rounded-b-[2.5rem] overflow-x-auto"><table className="w-full text-left text-[11px]"><thead className="bg-slate-50 text-slate-400 font-black uppercase border-b text-[9px]"><tr><th className="p-8">Campaign Details</th><th className="text-center">Code</th><th className="text-center">Benefit</th><th className="text-right pr-8">Status</th></tr></thead><tbody className="divide-y divide-slate-50 font-bold">{coupons.map(c => (<tr key={c._id}><td className="p-8"><div className="text-[#1A5F7A] font-black uppercase text-[12px]">{c.description || "No Narrative"}</div><div className="text-slate-400 text-[10px] mt-1">{c.courseCode} | Exp: {new Date(c.validTo).toLocaleDateString()}</div></td><td className="text-center text-[#F37021] italic uppercase font-black text-[14px]">{c.code}</td><td className="text-center font-black text-[13px]">{c.discountType === 'PERCENTAGE' ? `${c.discountValue}%` : `₹${c.discountValue}`} OFF</td><td className="text-right pr-8"><div className="bg-green-100 text-green-600 px-3 py-1 rounded-full text-[8px] font-black uppercase border border-green-200 ml-auto w-fit">ACTIVE</div></td></tr>))}</tbody></table></div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'logs' && (
                        <div className="space-y-8">
                             ...
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden overflow-x-auto">
                                <table className="w-full text-left"><thead className="bg-slate-50 text-[10px] font-black uppercase border-b p-8 text-slate-400"><tr><th className="p-8">Operator</th><th>Action</th><th>Target</th><th className="pr-8 text-right">Timestamp</th></tr></thead><tbody className="divide-y divide-slate-50 font-bold text-[11px]">{auditLogs.map((log, idx) => (<tr key={idx} className="hover:bg-slate-50 transition-all"><td className="p-8"><div className="text-[#1A5F7A] font-black uppercase italic">{log.performedBy}</div></td><td className="uppercase opacity-60">{log.action}</td><td className="italic text-slate-500 font-black">{log.targetName}</td><td className="pr-8 text-right"><div className="text-[#1A5F7A] font-black text-[14px]">{new Date(log.timestamp).toLocaleTimeString()}</div><div className="text-[10px] text-slate-400 uppercase">{new Date(log.timestamp).toLocaleDateString()}</div></td></tr>))}</tbody></table>
                             </div>
                        </div>
                    )}
                </main>
                ...
                return (
                    <div className="fixed inset-0 z-[1000] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[3.5rem] p-10 max-w-md w-full shadow-2xl relative border-t-[15px] border-[#1A5F7A]">
                            ...
                            <h3 className="text-2xl font-black text-[#1A5F7A] uppercase italic mb-1">Ledger Control</h3>
                            ...
                            <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-8 font-black uppercase text-[10px]">
                                <button onClick={() => setDiscountMode(false)} className={`flex-1 py-3 rounded-xl transition-all ${!discountMode ? 'bg-white text-[#1A5F7A] shadow-sm' : 'text-slate-400'}`}>Receive Payment</button>
                            ...
                            <div className={`rounded-[2rem] p-6 mb-8 border flex justify-between items-center ${discountMode ? 'bg-orange-50 border-orange-100' : 'bg-slate-50 border-slate-100'}`}>
                                <div><p className="text-[9px] font-black text-slate-400 uppercase italic">Total</p><div className="text-xl font-black text-slate-600 italic">₹{ledger.total.toLocaleString()}</div></div>
                                <div className="text-right"><p className="text-[9px] font-black text-red-400 uppercase italic">Net Due</p><div className="text-2xl font-black italic text-red-600">₹{ledger.due.toLocaleString()}</div></div>
                            </div>

                            <form onSubmit={handleLedgerSync} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase ml-4 italic flex items-center gap-2">{discountMode ? <FiTag className="text-[#F37021]"/> : <FiDollarSign className="text-green-500"/>} Amount (₹)</label>
                                    <div className="relative shadow-inner rounded-[2rem] overflow-hidden">
                                ...
                                {discountMode && (
                                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-500 uppercase ml-4 italic flex items-center gap-2"><FiAlertCircle className="text-[#F37021]"/> Reason for Founder</label>
                                        <textarea required className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] font-bold text-xs outline-none focus:bg-white min-h-[100px] resize-none" placeholder="Provide reason..." value={discountData.reason} onChange={(e) => setDiscountData({...discountData, reason: e.target.value})} />
                                    </motion.div>
                                )}
                                <button type="submit" className={`w-full py-6 text-white rounded-[2rem] font-black uppercase text-xs tracking-[0.3em] shadow-xl flex items-center justify-center gap-3 transition-all active:scale-95 ${discountMode ? 'bg-[#F37021]' : 'bg-green-600'}`}>
                                    <FiCheckCircle size={18}/> {discountMode ? "Request Approval" : "Confirm Sync"}
                                </button>
                            </form>
                        </motion.div>
                    </div>
            ...
            <AnimatePresence>{logoutModal && (
                <div className="fixed inset-0 z-[1000] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl border-t-8 border-red-500 text-center">
                        ...
                        <h3 className="text-2xl font-black text-[#1A5F7A] uppercase italic mb-8">Terminate Session?</h3>
