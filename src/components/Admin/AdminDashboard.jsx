...
import { 
    ...
    FiZap, FiPlus, FiList, FiCheck, FiTrendingDown, FiMonitor
} from 'react-icons/fi';
    ...
    const handleCreateCoupon = async (e) => {
        ...
        try {
            const payload = {
                ...
                courseCode: couponForm.courseCode,
                validFrom: new Date(couponForm.validFrom),
                validTo: new Date(couponForm.validTo),
                isActive: true
            };
            const response = await axios.post(`${API_URL}/admin/coupons`, payload, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <div className="flex h-screen bg-slate-50 font-sans overflow-hidden text-left relative text-slate-800">
                                    ...
                                    <tbody className="divide-y divide-slate-50 font-bold">
                                        {filteredData.map(item => (
                                            <tr key={item._id} className="hover:bg-blue-50/20 transition-all group">
                                                <td className="p-7"><div className="font-black text-[#1A5F7A] uppercase italic text-[15px] group-hover:text-[#F37021]">{item.name}</div><div className="text-slate-400 font-bold text-[11px] mt-1 tracking-widest">{item.phone}</div></td>
                                                ...
                                                <td className="font-black text-slate-400 italic uppercase text-[11px]"><FiCalendar className="inline mr-2" />{new Date(item.createdAt).toLocaleDateString('en-GB')}</td>
                        ...
                        <div className="space-y-8 text-left animate-in fade-in slide-in-from-bottom-4">
                             ...
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden overflow-x-auto">
                                <table className="w-full text-left">
                                    <thead className="bg-slate-50 font-black uppercase text-slate-400 border-b tracking-widest text-[10px]"><tr><th className="p-7">Student Identity</th><th>Contact No.</th><th>Portal Access</th><th>Financial Ledger</th><th className="pr-7 text-right">Ledger Sync</th></tr></thead>
                                    <tbody className="divide-y divide-slate-50 font-bold">
                                        {filteredData.map(item => {
                                            ...
                                            return (
                                                <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                                                    <td className="p-7"><div className="font-black text-[#1A5F7A] uppercase italic text-[15px]">{item.name}</div><div className="text-slate-400 font-bold text-[12px] mt-0.5">{item.email}</div></td>
                                                    <td><div className="flex items-center gap-2 text-[#1A5F7A] font-black text-[14px]"><FiPhoneCall className="text-[#F37021]" size={14}/> {item.phone || "N/A"}</div></td>
                                                    <td>{item.isPortalActive ? <div className="flex items-center gap-2 text-green-600 bg-green-50 px-3 py-1 rounded-lg w-fit font-black text-[10px] border border-green-100 uppercase italic"><FiCheckCircle/> Active</div> : <button onClick={async () => { await axios.patch(`${API_URL}/admin/registrations/${item._id}/grant-access`, {}, { headers: { Authorization: `Bearer ${token}` } }); fetchData(true); }} className="bg-[#1A5F7A] text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase italic shadow-lg active:scale-95">Unlock Access</button>}</td>
                                                    <td><div className="font-black"><div className="text-[#1A5F7A] text-[15px]">₹{item.amountPaid?.toLocaleString()} / <span className="text-slate-300">₹{ledger.total.toLocaleString()}</span></div><div className={`text-[11px] uppercase italic ${ledger.due > 0 ? 'text-red-500' : 'text-green-600'}`}>{isPending ? "APPROVAL PENDING" : (ledger.due > 0 ? `Due: ₹${ledger.due}` : "Cleared")}</div></div></td>
                                                    <td className="pr-7 text-right">
                                                        <div className="flex items-center justify-end gap-3">{userRole === 'founder' && isPending && <button onClick={() => handleFounderApprove(item._id)} className="bg-green-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg hover:scale-105 transition-all"><FiCheck className="inline mr-1"/> Approve</button>}
                                                        <button onClick={() => setPaymentModal({ show: true, student: item, amount: "" })} className="w-12 h-12 bg-[#F37021]/10 text-[#F37021] rounded-2xl flex items-center justify-center hover:bg-[#F37021] hover:text-white transition-all shadow-sm"><FiCreditCard size={20}/></button></div>
                                                    </td>
                                                </tr>
                                            );
                                        })}
                ...
                return (
                    <div className="fixed inset-0 z-[1000] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 text-left">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[3.5rem] p-10 max-w-md w-full shadow-2xl relative border-t-[15px] border-[#1A5F7A]">
                            ...
                            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-6 italic">Updating Student: {paymentModal.student.name}</p>
            ...
            <AnimatePresence>{logoutModal && (
                <div className="fixed inset-0 z-[1000] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 text-center">
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl border-t-8 border-red-500"><FiLogOut className="mx-auto text-red-500 mb-6" size={48} /><h3 className="text-2xl font-black text-[#1A5F7A] uppercase italic mb-8 leading-tight tracking-tighter">Terminate Active<br/>Management Session?</h3><div className="grid grid-cols-2 gap-4"><button onClick={() => setLogoutModal(false)} className="py-4 bg-slate-100 rounded-2xl font-black uppercase text-[10px] text-slate-500 active:scale-95 transition-all hover:bg-slate-200">Cancel</button><button onClick={handleLogout} className="py-4 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] shadow-xl active:scale-95 transition-all hover:bg-red-600">Logout Now</button></div></motion.div>
                </div>
            )}</AnimatePresence>
        </div>
    );
