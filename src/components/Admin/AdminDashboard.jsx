...
import { 
    ...
    FiZap, FiPlus, FiList, FiCheck, FiTrendingDown, FiMonitor, FiCopy, FiChevronDown
} from 'react-icons/fi';
...
export default function AdminDashboard() {
    ...
    const [finances, setFinances] = useState({ total: 0, monthlyPayout: 0, pendingAdjustments: 0 });
    ...
    const getNormalizedEnrollments = useCallback((student) => {
        ...
        return list.map(en => ({
            ...en,
            courseFee: en.courseFee || student.totalFee || 0,
    ...
    const analyzeFinances = useCallback((studentList) => {
        ...
        studentList.forEach(s => {
            totalRev += (Number(s.amountPaid) || 0);
            getNormalizedEnrollments(s).forEach(en => {
                const enDate = new Date(en.enrolledAt);
                if (enDate.getMonth() === now.getMonth() && enDate.getFullYear() === now.getFullYear() && en.paymentStatus === 'VERIFIED') {
                    monthly += (Number(en.courseFee) || 0);
                }
            });
        });
        const pending = studentList.filter(s => s.discountRequest?.status === 'PENDING').length;
        setFinances(prev => ({ ...prev, total: totalRev, monthlyPayout: monthly, pendingAdjustments: pending }));
    }, [getNormalizedEnrollments]);
    ...
    const handleEnquiryStatusUpdate = async (id, currentStatus, currentRemarks, studentName) => {
        ...
        const finalRemarks = newStatus ? "CONTACTED" : "NOT CONTACTED";
        try {
    ...
    const memoizedSidebar = useMemo(() => (
        <nav className="flex flex-col gap-3 flex-1 no-scrollbar overflow-y-auto">
            ...
            {hasAccess('overview') && <SidebarBtn active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} icon={<FiGrid />} label="Dashboard" />}
            {hasAccess('enquiries') && <SidebarBtn active={activeTab === 'enquiries'} onClick={() => setActiveTab('enquiries')} icon={<FiMessageSquare />} label="Leads" />}
            {hasAccess('registrations') && <SidebarBtn active={activeTab === 'registrations'} onClick={() => setActiveTab('registrations')} icon={<FiUsers />} label="Registry" />}
            <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] mt-6 mb-2 ml-4">LMS Controls</p>
            {hasAccess('batches') && <SidebarBtn active={activeTab === 'batches'} onClick={() => setActiveTab('batches')} icon={<FiClock />} label="Batches" />}
            <SidebarBtn active={activeTab === 'lectures'} onClick={() => setActiveTab('lectures')} icon={<FiVideo />} label="Live Class" />
            <SidebarBtn active={activeTab === 'materials'} onClick={() => setActiveTab('materials')} icon={<FiBookOpen />} label="Vault" />
            <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] mt-6 mb-2 ml-4">Admin Tools</p>
            {hasAccess('coupons') && <SidebarBtn active={activeTab === 'coupons'} onClick={() => setActiveTab('coupons')} icon={<FiTag />} label="Coupons" />}
        ...
        <div className="flex h-screen bg-slate-50 font-sans overflow-hidden text-left relative text-slate-800">
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                <header className="bg-white h-24 px-10 flex items-center justify-between border-b border-slate-100 shadow-sm sticky top-0 z-[100]">
                    <div className="flex items-center gap-5"><button className="lg:hidden text-[#1A5F7A]" onClick={() => setIsSidebarOpen(true)}><FiMenu size={24} /></button><h2 className="font-black text-[#1A5F7A] text-lg uppercase italic">{activeTab.replace('-', ' ')}</h2></div>
                    <div className="flex items-center gap-8">
                        <div className="hidden sm:block text-right pr-6 border-r border-slate-100"><span className="text-[#1A5F7A] text-[13px] font-black uppercase block">{userName}</span><span className="text-[9px] font-bold text-[#F37021] uppercase opacity-70">{userRole}</span></div>
                ...
                <main className="p-12 overflow-y-auto flex-1 no-scrollbar bg-[#F8FAFC]">
                    {activeTab === 'overview' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left animate-in fade-in duration-500">
                            <div className="bg-[#1A5F7A] text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
                                ...
                                <p className="text-[10px] uppercase font-black opacity-60 mb-1">Gross Collection</p>
                                <div className="text-4xl font-black italic tracking-tighter">₹{finances.total.toLocaleString()}</div>
                            </div>
                            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 flex items-center gap-5 hover:border-[#F37021] transition-all">
                                <div className="p-4 bg-orange-50 text-[#F37021] rounded-2xl"><FiActivity size={30}/></div>
                                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Monthly</p><div className="text-xl font-black text-[#1A5F7A] italic leading-none">₹{finances.monthlyPayout.toLocaleString()}</div></div>
                            </div>
                            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 flex items-center gap-5 hover:border-[#1A5F7A] transition-all">
                                <div className="p-4 bg-blue-50 text-blue-500 rounded-2xl"><FiUsers size={30}/></div>
                                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Registry</p><div className="text-xl font-black text-[#1A5F7A] italic leading-none">{students.length} Students</div></div>
                            </div>
                            <div className="bg-white p-8 rounded-[2.5rem] border border-red-100 flex items-center gap-5 hover:bg-red-50 transition-all border-dashed">
                                <div className="p-4 bg-red-50 text-red-500 rounded-2xl"><FiTrendingDown size={30}/></div>
                                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Queue</p><div className="text-xl font-black text-[#1A5F7A] italic leading-none">{finances.pendingAdjustments} Alerts</div></div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'enquiries' && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                             <div className="flex justify-between items-center gap-4"><h3 className="text-3xl font-black text-[#1A5F7A] uppercase italic leading-none">Inquiry Pipeline</h3><div className="relative w-96 group"><FiSearch className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300"/><input type="text" placeholder="Search Identity..." className="w-full pl-14 pr-6 py-4 bg-white border border-slate-200 rounded-[1.5rem] outline-none font-bold focus:border-[#F37021] transition-all" onChange={e => setSearchQuery(e.target.value)}/></div></div>
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden overflow-x-auto">
                                <table className="w-full text-left text-[11px]"><thead className="bg-slate-50 font-black uppercase text-slate-400 border-b tracking-widest text-[9px]"><tr><th className="p-7">Lead Identity</th><th>Program</th><th>Source</th><th>Date</th><th className="pr-7 text-right">Action</th></tr></thead>
                                    <tbody className="divide-y divide-slate-50 font-bold">
                                        {filteredEnquiries.map(item => (
                                            <tr key={item._id} className="hover:bg-blue-50/20 transition-all group">
                                                ...
                                                <td className="text-slate-400 italic uppercase text-[11px]"><FiCalendar className="inline mr-2"/>{new Date(item.createdAt).toLocaleDateString('en-GB')}</td>
                    ...
                    {activeTab === 'registrations' && (
                        <div className="space-y-8 animate-in fade-in duration-500">
                             <div className="flex justify-between items-center gap-4"><h3 className="text-3xl font-black text-[#1A5F7A] uppercase italic leading-none">Student Registry</h3><div className="relative w-96 group shadow-sm"><FiSearch className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300"/><input type="text" placeholder="Search Identity or UTR..." className="w-full pl-14 pr-6 py-4 bg-white border border-slate-200 rounded-[1.5rem] outline-none font-bold focus:border-[#F37021] transition-all" onChange={e => setSearchQuery(e.target.value)}/></div></div>
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden overflow-x-auto">
                                <table className="w-full text-left"><thead className="bg-slate-50 font-black uppercase text-slate-400 border-b tracking-widest text-[9px]"><tr><th className="p-7">Profile Identity</th><th>Financial standing</th><th>Portal Status</th><th className="pr-7 text-right">Ledger Sync</th></tr></thead>
                                            ...
                                            return (
                                                <React.Fragment key={student._id}>
                                                    <tr className={`group transition-all ${isExpanded ? 'bg-blue-50/30' : 'hover:bg-slate-50/50'}`}>
                                                        <td className="p-7"><div className="font-black text-[#1A5F7A] uppercase italic text-[16px] leading-none">{student.name}</div><div className="text-slate-400 font-bold text-[10px] uppercase mt-1 italic">{enrollments.length} Enrollment(s)</div></td>
                                                        <td><div className="font-black text-[#1A5F7A] text-[15px]">₹{student.amountPaid?.toLocaleString()} <span className="text-slate-300 text-[11px] font-normal">/ ₹{ledger.total.toLocaleString()}</span></div><div className={`text-[10px] uppercase italic mt-1 ${ledger.due > 0 ? 'text-red-500' : 'text-green-600'}`}>{isPending ? "APPROVAL REQUIRED" : (ledger.due > 0 ? `DUE: ₹${ledger.due}` : 'CLEARED')}</div></td>
                                                        <td><div className={`px-3 py-1.5 rounded-xl w-fit font-black text-[9px] border ${student.isPortalActive ? 'bg-green-50 text-green-600 border-green-100' : 'bg-slate-100 text-slate-300 border-slate-200'}`}>{student.isPortalActive ? 'PORTAL ACTIVE' : 'LOCKED'}</div></td>
                                                    ...
                                                    <AnimatePresence>{isExpanded && (
                                                        <tr><td colSpan="4" className="p-0 bg-white"><motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0 }} className="overflow-hidden border-b-8 border-blue-50/50">
                                                                    ...
                                                                    <div key={i} className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-6 hover:border-[#F37021] transition-all">
                                                                        <div className="flex items-center gap-6">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <div className="flex items-center gap-2 text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-widest"><FiCopy className="cursor-pointer hover:text-[#F37021]" onClick={() => {navigator.clipboard.writeText(en.transactionId); triggerToast("COPIED")}}/>UTR: <span className="text-slate-600 font-black">{en.transactionId}</span>{en.isLegacy && <span className="ml-2 bg-indigo-50 text-indigo-500 px-2 py-0.5 rounded-lg text-[8px]">LEGACY</span>}</div>
                                                                            </div>
                                                                        </div>
                                                                        <div className="flex items-center gap-8">
                                                                            <div className="text-right pr-6 border-r border-slate-100"><p className="text-[8px] font-black text-slate-300 uppercase">Valuation</p><p className="text-[14px] font-black text-[#1A5F7A]">₹{en.courseFee?.toLocaleString()}</p></div>
                                                                            {en.paymentStatus === 'PENDING' ? <button onClick={() => handleApprovePayment(student._id, student.name, en.transactionId)} className="bg-green-600 text-white px-7 py-3 rounded-2xl text-[10px] font-black uppercase italic shadow-lg hover:bg-green-700 transition-all flex items-center gap-2"><FiShield /> Verify UTR</button> : <div className="text-green-600 font-black text-[10px] uppercase italic flex items-center gap-2 px-4"><FiCheckCircle/> Verified</div>}
                                                                        </div>
                                                                    </div>
                                                                ))}
                                                            </div></motion.div></td></tr>
                                                    )}</AnimatePresence>
                                                </React.Fragment>
                                            );
                                        })}
                        ...
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden border-t-[10px] border-[#1A5F7A]">
                                <div className="p-8 border-b flex items-center justify-between bg-slate-50/50">
                                    <div className="flex items-center gap-4"><div className="w-2 h-10 bg-[#F37021] rounded-full"></div><h3 className="text-xl font-black text-[#1A5F7A] uppercase italic leading-none">Coupon Deployment Engine</h3></div>
                                </div>
                                <form onSubmit={handleCreateCoupon} className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest italic">Activation Code</label><input required className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold outline-none focus:bg-white focus:border-[#F37021] transition-all uppercase tracking-widest" placeholder="SALE2026" value={couponForm.code} onChange={e => setCouponForm({...couponForm, code: e.target.value})} /></div>
                                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest italic">Usage Limit</label><input required type="number" className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold outline-none focus:bg-white focus:border-[#F37021]" placeholder="100" value={couponForm.maxUsage} onChange={e => setCouponForm({...couponForm, maxUsage: e.target.value})} /></div>
                                    
                                    <div className="col-span-2 space-y-2">
                                        <label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest italic">Campaign Description / Narrative</label>
                                        <textarea required className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold outline-none focus:bg-white focus:border-[#F37021] transition-all min-h-[100px] resize-none" placeholder="e.g. Summer Special 20% Off for MERN Stack students..." value={couponForm.description} onChange={e => setCouponForm({...couponForm, description: e.target.value})} />
                                    </div>

                                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest italic">Start Date</label><input required type="date" className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold outline-none focus:bg-white focus:border-[#F37021]" value={couponForm.validFrom} onChange={e => setCouponForm({...couponForm, validFrom: e.target.value})} /></div>
                                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest italic">Expiry Date</label><input required type="date" className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold outline-none focus:bg-white focus:border-[#F37021]" value={couponForm.validTo} onChange={e => setCouponForm({...couponForm, validTo: e.target.value})} /></div>
                                    
                                    <div className="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-10 mt-6 border-t pt-10">
                                        <div className="space-y-2"><label className="text-[10px] font-black uppercase text-[#F37021] ml-4 flex items-center gap-2 italic tracking-widest"><FiTarget/> Product Scope</label><select className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold outline-none focus:bg-white focus:border-[#F37021] appearance-none" value={couponForm.courseCode} onChange={e => setCouponForm({...couponForm, courseCode: e.target.value})}><option value="ALL">ALL PROGRAMS</option>{allCourses.map(c => <option key={c.title} value={c.title}>{c.title}</option>)}</select></div>
                                        <div className="space-y-2"><label className="text-[10px] font-black uppercase text-[#F37021] ml-4 flex items-center gap-2 italic tracking-widest"><FiPieChart/> Mode</label><div className="flex bg-slate-100 p-1.5 rounded-[2rem]"><button type="button" onClick={() => setCouponForm({...couponForm, discountType: 'PERCENTAGE'})} className={`flex-1 py-3 rounded-full font-black text-[10px] transition-all ${couponForm.discountType === 'PERCENTAGE' ? 'bg-[#1A5F7A] text-white shadow-xl' : 'text-slate-400'}`}>% Percentage</button><button type="button" onClick={() => setCouponForm({...couponForm, discountType: 'FLAT'})} className={`flex-1 py-3 rounded-full font-black text-[10px] transition-all ${couponForm.discountType === 'FLAT' ? 'bg-[#1A5F7A] text-white shadow-xl' : 'text-slate-400'}`}>₹ Fixed</button></div></div>
                                        <div className="col-span-2 space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-4 italic tracking-widest">Deployment Value</label><div className="relative group shadow-inner rounded-[3rem] overflow-hidden"><input required type="number" className="w-full p-10 bg-slate-50 border-2 border-slate-100 rounded-[3rem] font-black text-6xl outline-none text-center focus:bg-white focus:border-[#F37021] transition-all" placeholder="00" value={couponForm.discountValue} onChange={e => setCouponForm({...couponForm, discountValue: e.target.value})} /><div className="absolute right-10 top-1/2 -translate-y-1/2 text-4xl font-black text-slate-200">{couponForm.discountType === 'PERCENTAGE' ? '%' : '₹'}</div></div></div>
                                    </div>
                             ...
                             <div className="bg-[#1A5F7A] rounded-[2.5rem] shadow-xl overflow-hidden">
                                <div className="p-8 flex justify-between items-center text-white"><h3 className="font-black uppercase italic tracking-widest">Active Production Registry</h3><div className="bg-[#F37021] px-4 py-1.5 rounded-full text-[10px] font-black uppercase border border-white/20">{coupons.length} LIVE</div></div>
                                <div className="bg-white mx-1 mb-1 rounded-b-[2.5rem] overflow-hidden overflow-x-auto"><table className="w-full text-left text-[11px]"><thead className="bg-slate-50 text-slate-400 font-black uppercase border-b text-[9px] tracking-widest"><tr><th className="p-8">Campaign Details</th><th className="text-center">Code</th><th className="text-center">Benefit</th><th className="text-right pr-8">Status</th></tr></thead><tbody className="divide-y divide-slate-50 font-bold">{coupons.map(c => (<tr key={c._id}><td className="p-8"><div className="text-[#1A5F7A] font-black uppercase text-[12px]">{c.description || "No Narrative"}</div><div className="text-slate-400 text-[10px] mt-1">{c.courseCode} | {new Date(c.validTo).toLocaleDateString()}</div></td><td className="text-center text-[#F37021] italic uppercase font-black text-[14px]">{c.code}</td><td className="text-center font-black text-[13px]">{c.discountType === 'PERCENTAGE' ? `${c.discountValue}%` : `₹${c.discountValue}`} OFF</td><td className="text-right pr-8"><div className="bg-green-100 text-green-600 px-3 py-1 rounded-full text-[8px] font-black uppercase italic border border-green-200 ml-auto w-fit">ACTIVE</div></td></tr>))}</tbody></table></div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'logs' && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                             <div className="flex justify-between items-center"><h3 className="text-3xl font-black text-[#1A5F7A] uppercase italic">Audit Trail</h3><div className="bg-[#1A5F7A] text-white px-6 py-2 rounded-full font-black text-[10px] uppercase italic border border-white/20"><FiShield className="inline mr-2 text-green-500 animate-pulse"/> Sessions Verified</div></div>
                             <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[10px] font-black uppercase border-b p-8 tracking-widest text-slate-400"><tr><th className="p-8">Operator</th><th>Action</th><th>Target</th><th className="pr-8 text-right">Timestamp</th></tr></thead><tbody className="divide-y divide-slate-50 font-bold text-[11px]">{auditLogs.map((log, idx) => (<tr key={idx} className="hover:bg-slate-50 border-l-4 border-transparent hover:border-[#F37021] transition-all"><td className="p-8"><div className="text-[#1A5F7A] font-black uppercase italic">{log.performedBy}</div></td><td className="uppercase opacity-60 text-[10px]">{log.action}</td><td className="italic text-slate-500 font-black">{log.targetName}</td><td className="pr-8 text-right"><div className="text-[#1A5F7A] font-black text-[14px]">{new Date(log.timestamp).toLocaleTimeString()}</div><div className="text-[10px] text-slate-400 uppercase tracking-widest">{new Date(log.timestamp).toLocaleDateString()}</div></td></tr>))}</tbody></table></div>
                        </div>
                    )}
                </main>
            </div>
                ...
                return (
                    <div className="fixed inset-0 z-[1000] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[3.5rem] p-10 max-w-md w-full shadow-2xl relative border-t-[15px] border-[#1A5F7A]">
                            ...
                            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-6 italic">Updating: {paymentModal.student.name}</p>
                            ...
                            <div className={`rounded-[2rem] p-6 mb-8 border flex justify-between items-center ${discountMode ? 'bg-orange-50 border-orange-100' : 'bg-slate-50 border-slate-100'}`}><div><p className="text-[9px] font-black text-slate-400 uppercase mb-1 italic">Total</p><div className="text-xl font-black text-slate-600 italic tracking-tighter">₹{ledger.total.toLocaleString()}</div></div><div className="text-right"><p className="text-[9px] font-black text-red-400 uppercase mb-1 italic tracking-widest">Net Due</p><div className="text-2xl font-black italic text-red-600 tracking-tighter leading-none">₹{ledger.due.toLocaleString()}</div></div></div>
                            <form onSubmit={handleLedgerSync} className="space-y-6"><div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase ml-4 tracking-widest italic flex items-center gap-2">{discountMode ? <FiTag className="text-[#F37021]"/> : <FiDollarSign className="text-green-500"/>} Amount (₹)</label><div className="relative group shadow-inner rounded-[2rem] overflow-hidden"><input required autoFocus type="number" className={`w-full p-6 bg-slate-50 border-2 rounded-[2rem] font-black text-3xl outline-none text-center ${discountMode ? 'focus:border-[#F37021] focus:bg-white' : 'focus:border-green-500 focus:bg-white'}`} placeholder="0000" value={discountMode ? discountData.amount : paymentModal.amount} onChange={(e) => discountMode ? setDiscountData({...discountData, amount: e.target.value}) : setPaymentModal({...paymentModal, amount: e.target.value})} /><div className="absolute left-6 top-1/2 -translate-y-1/2 text-xl font-black text-slate-200">₹</div></div></div>{discountMode && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase ml-4 italic flex items-center gap-2"><FiAlertCircle className="text-[#F37021]"/> Reason for Founder</label><textarea required className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] font-bold text-xs outline-none focus:bg-white focus:border-[#F37021] min-h-[100px] resize-none" placeholder="Provide reason..." value={discountData.reason} onChange={(e) => setDiscountData({...discountData, reason: e.target.value})} /></motion.div>)}<button type="submit" className={`w-full py-6 text-white rounded-[2rem] font-black uppercase text-xs tracking-[0.3em] shadow-xl flex items-center justify-center gap-3 transition-all active:scale-95 ${discountMode ? 'bg-[#F37021] hover:bg-[#d9621c]' : 'bg-green-600 hover:bg-green-700'}`}><FiCheckCircle size={18}/> {discountMode ? "Request Approval" : "Confirm Sync"}</button></form>
                        </motion.div>
                    </div>
                );
            })()}</AnimatePresence>
            ...
            <AnimatePresence>{logoutModal && (
                <div className="fixed inset-0 z-[1000] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl border-t-8 border-red-500 text-center">
                        ...
                        <h3 className="text-2xl font-black text-[#1A5F7A] uppercase italic mb-8 tracking-tighter">Terminate Active<br/>Management Session?</h3>
                        <div className="grid grid-cols-2 gap-4"><button onClick={() => setLogoutModal(false)} className="py-4 bg-slate-100 rounded-2xl font-black uppercase text-[10px] text-slate-500">Cancel</button><button onClick={handleLogout} className="py-4 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] shadow-xl">Logout</button></div>
                    </motion.div>
                </div>
            )}</AnimatePresence>
        </div>
